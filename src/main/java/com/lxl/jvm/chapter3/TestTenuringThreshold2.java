 ...
 * -XX:MaxTenuringThreshold=1（设置几次回收后进入老年代）
 * 如果survivor空间的相同年龄所有对象大小的总和大于survivor空间的一半，年龄大于或等于该年龄的对象，进入老年代，无需等待
 *
 *-verbose:gc -Xms20M -Xmx20M -Xmn10M -XX:+PrintGCDetails -XX:SurvivorRatio=8 -XX:MaxTenuringThreshold=15（设置几次回收后进入老年代） -XX:+PrintTenuringDistribution
 * Heap
